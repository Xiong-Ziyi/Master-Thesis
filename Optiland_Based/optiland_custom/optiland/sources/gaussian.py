...
class GaussianSource(BaseSource):
    ...
    def generate_rays(self, num_rays: int) -> RealRays:
        ...
        L_initial = self.sigma_angular_rad_x * sqrt2 * be.erfinv(2 * u_samples[:, 2] - 1)
        M_initial = self.sigma_angular_rad_y * sqrt2 * be.erfinv(2 * u_samples[:, 3] - 1)

        # Filter for physically possible rays (L² + M² < 1)
        ...
        # --- CORRECT POWER ASSIGNMENT ---
        # With importance sampling, the ray density already models the beam profile.
        # Therefore, each ray must carry an equal amount of power.
        ...
        rays = RealRays(
            ...
            M=M_initial,
            N=N_initial,
            intensity=intensity_power_array, # This now holds the correct, equal power per ray.
