...
while 1
  ...
  switch Directive
      ...
      case 'SAY', ApertureData = zGetSystemAper; EPD = ApertureData(3);
                  ...
                  Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('APER 0 1 %11.9f 0', EPD); 
                  [Token, Rest] = strtok(Rest, ' ,'); EPD = 2 * str2double(Token);
                  Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                  Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('APER 0 %i %11.9f 0',Config, EPD); 
      case 'SCY', [Token, Rest] = strtok(Rest, ' ,');
                  if (strcmp(Token, 'FANG'))
                      Angle = zGetField(2); Angle = Angle(2);
                      Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                      Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('YFIE 2 1 %11.9f 0', Angle); 
                      [Token, Rest] = strtok(Rest, ' ,');
                      Angle = str2double(Token);
                      Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                      Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('YFIE 2 %i %11.9f 0', Config, Angle); 
                  end;
      case 'CV', [Token, Rest] = strtok(Rest, ' ,'); ACCOSSurface = str2num(Token); ZEMAXSurface = zFindLabel(ACCOSSurface);
                 ...
                 Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                 Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('CRVT %i 1 %11.9f 0', ZEMAXSurface, Curvature);
                 [Token, Rest] = strtok(Rest, ' ,'); Curvature = str2double(Token);
                 Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                 Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('CRVT %i %i %11.9f 0', ZEMAXSurface, Config, Curvature);
      case 'CC', [Token, Rest] = strtok(Rest, ' ,'); ACCOSSurface = str2num(Token); ZEMAXSurface = zFindLabel(ACCOSSurface);
                 ...
                 Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                 Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('CONN %i 1 %11.9f 0', ZEMAXSurface, Conic);
                 [Token, Rest] = strtok(Rest, ' ,'); Conic = str2double(Token);
                 Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                 Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('CONN %i %i %11.9f 0', ZEMAXSurface, Config, Conic);
      case 'AD', [Token, Rest] = strtok(Rest, ' ,'); ACCOSSurface = str2num(Token); ZEMAXSurface = zFindLabel(ACCOSSurface);
                 ...
                 Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                 Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('PAR2 %i 1 %11.9f 0', ZEMAXSurface, AD);
                 [Token, Rest] = strtok(Rest, ' ,'); AD = str2double(Token);
                 Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                 Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('PAR2 %i %i %11.9f 0', ZEMAXSurface, Config, AD);
      case 'AE', [Token, Rest] = strtok(Rest, ' ,'); ACCOSSurface = str2num(Token); ZEMAXSurface = zFindLabel(ACCOSSurface);
                 ...
                 Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                 Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('PAR3 %i 1 %11.9f 0', ZEMAXSurface, AE);
                 [Token, Rest] = strtok(Rest, ' ,'); AE = str2double(Token);
                 Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                 Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('PAR3 %i %i %11.9f 0', ZEMAXSurface, Config, AE);
      case 'AF', [Token, Rest] = strtok(Rest, ' ,'); ACCOSSurface = str2num(Token); ZEMAXSurface = zFindLabel(ACCOSSurface);
                 ...
                 Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                 Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('PAR4 %i 1 %11.9f 0', ZEMAXSurface, AF);
                 [Token, Rest] = strtok(Rest, ' ,'); AF = str2double(Token);
                 Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                 Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('PAR4 %i %i %11.9f 0', ZEMAXSurface, Config, AF);
      case 'AG', [Token, Rest] = strtok(Rest, ' ,'); ACCOSSurface = str2num(Token); ZEMAXSurface = zFindLabel(ACCOSSurface);
                 ...
                 Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                 Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('PAR5 %i 1 %11.9f 0', ZEMAXSurface, AG);
                 [Token, Rest] = strtok(Rest, ' ,'); AG = str2double(Token);
                 Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                 Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('PAR5 %i %i %11.9f 0', ZEMAXSurface, Config, AG);
      case 'TH', [Token, Rest] = strtok(Rest, ' ,'); ACCOSSurface = str2num(Token); ZEMAXSurface = zFindLabel(ACCOSSurface);
                 ...
                 Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                 Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('THIC %i 1 %11.9f 0', ZEMAXSurface, Thickness);
                 [Token, Rest] = strtok(Rest, ' ,'); Thickness = str2double(Token);
                 Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                 Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('THIC %i %i %11.9f 0', ZEMAXSurface, Config, Thickness);
      case 'EOS', return;
  ...
  switch Directive
      ...
      case 'WV', [Token, Rest] = strtok(Rest, ' ,'); WaveNumber = 0;
                 while (Token)
                     ...
                     Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                     Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('WAVE %i 1 %11.9f 0', WaveNumber, WV);
                     WV = str2double(Token);
                     Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                     Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('WAVE %i %i %11.9f 0', WaveNumber, Config, WV);
                     [Token, Rest] = strtok(Rest, ' ,');
                 end;
     case 'SPTWT',[Token, Rest] = strtok(Rest, ' ,'); WaveNumber = 0;
                 while (Token)
                     ...
                     Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                     Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('WLWT %i 1 %11.9f 0', WaveNumber, Weight);
                     Weight = str2double(Token);
                     Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                     Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('WLWT %i %i %11.9f 0', WaveNumber, Config, Weight);
                     [Token, Rest] = strtok(Rest, ' ,');
                 end;
      ...
      case 'CLAP',[Token, Rest] = strtok(Rest, ' ,');
                 ...
                 Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                 Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('APMX %i 1 %11.9f 0', ZEMAXSurface, Apmx);
                 Apmx = str2double(Token);
                 Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                 Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('APMX %i %i %11.9f 0', ZEMAXSurface, Config, Apmx);
      ...
      case 'RD', Curvature = zGetSurfaceData(ZEMAXSurface, 2);
                 ...
                 Fixups.ConfigOperands = Fixups.ConfigOperands + 1;
                 if (Radius ~= 0)
                   Fixups.ConfigData{Fixups.ConfigOperands} = sprintf('CRVT %i %i %11.9f 0', ZEMAXSurface, Config, 1/Radius); 
                 else
