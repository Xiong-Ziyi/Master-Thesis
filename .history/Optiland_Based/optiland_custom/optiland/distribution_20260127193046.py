...
class AnnularDistribution(BaseDistribution):
    """
    AnnularDistribution class for generating points within an annular region
    using a hexagonal (angular-arm) distribution.

    Generates points arranged on concentric rings between an inner and outer
    radius, with a fixed number of angular arms.

    By Ziyi Xiong 2025/12
    
    Args:
        r_inner (float): Normalized inner radius (0 <= r_inner < r_outer)
        r_outer (float): Normalized outer radius (r_inner < r_outer <= 1)
        num_arms (int): Number of angular arms (default: 6)
        num_rings (int): Number of concentric rings (default: 3)
    """
    ...
    def __init__(
        ...
        if not (0.0 <= r_inner < r_outer <= 1.0):
            raise ValueError("Radii must satisfy 0 <= r_inner < r_outer <= 1.0")
    ...
    def generate_points(self):
        """
        Generate points in an annular region with a hexagonal-style distribution.
        """
        x_list = []
        y_list = []

        # Angular positions
        theta = be.linspace(0, 2 * be.pi, self.num_arms + 1)[:-1]

        # Radial positions
        radii = be.linspace(self.r_inner, self.r_outer, self.num_rings)

        # Handle central point if inner radius is zero
        if self.r_inner == 0.0:
            x_list.append(0.0)
            y_list.append(0.0)
            start_ring_idx = 1
        ...
        # Generate rings
        for i in range(start_ring_idx, len(radii)):
            r_current = radii[i]
            x_list.append(r_current * be.cos(theta))
            y_list.append(r_current * be.sin(theta))

        # Flatten lists into arrays
        self.x = be.concatenate([be.atleast_1d(be.array(x)) for x in x_list])
        self.y = be.concatenate([be.atleast_1d(be.array(y)) for y in y_list])

class GaussianDistribution(BaseDistribution):
    """A class representing a truncated 2D Gaussian distribution.

    Generates `num_points` deterministically on concentric rings with evenly
    spaced arms. Ring radii follow a truncated Gaussian profile with standard
    deviation `sigma` inside the unit disk.
    ...
    def __init__(self, sigma: float = 0.25):
        super().__init__()
        if sigma <= 0:
            raise ValueError("Gaussian distribution sigma must be positive.")
        
        self.sigma = sigma

    def generate_points(self, num_points: int = 25):
        """Generates Gaussian-distributed points within the unit disk.
        ...
        num_points_sqrt = be.ceil(be.sqrt(num_points))
        num_points = int(num_points_sqrt)**2

        num_rings = max(1, int(be.sqrt(num_points)))
        num_arms = max(1, int(be.ceil(num_points / num_rings)))

        u = (be.arange(num_rings) + 0.5) / num_rings
        trunc = 1.0 - be.exp(-(1.0 / (2 * self.sigma**2)))
        radius = self.sigma * be.sqrt(-2.0 * be.log(1.0 - u * trunc))

        theta = be.linspace(0, 2 * be.pi, num_arms + 1)[:-1]
        r_grid, t_grid = be.meshgrid(radius, theta)
        x_all = (r_grid * be.cos(t_grid)).reshape(-1)
        y_all = (r_grid * be.sin(t_grid)).reshape(-1)

        self.x = x_all[:num_points]
        self.y = y_all[:num_points]
...
def create_distribution(distribution_type: DistributionType) -> BaseDistribution:
    ...
    distribution_classes: dict[
        ...
        "annular": AnnularDistribution, # newly added by Ziyi Xiong
