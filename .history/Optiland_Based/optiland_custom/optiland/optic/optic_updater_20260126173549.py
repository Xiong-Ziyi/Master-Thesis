...
from optiland.geometries import Plane, SegmentedSurface, StandardGeometry
...
class OpticUpdater:
    ...
    #################### Methods for segmented surface variables ####################
    def set_cv1(self, value, surface_number):
        """Set the center zone's radius of a segmented surface

        by Ziyi Xiong 2025/12
        
        Args:
            value (float): The new value for the cv1 coefficient.
            surface_number (int): The index of the surface to modify.
        """
        surface = self.optic.surface_group.surfaces[surface_number]
        
        if not isinstance(surface.geometry, SegmentedSurface):
            raise TypeError(
                f"Surface {surface_number} is not a SegmentedSurface."
            )
        else:
            surface.geometry.cv1 = value
        
    def set_cv2(self, value, surface_number):
        """Set the ring zone's radius of a segmented surface

        by Ziyi Xiong 2025/12
        
        Args:
            value (float): The new value for the cv2 coefficient.
            surface_number (int): The index of the surface to modify.
        """
        surface = self.optic.surface_group.surfaces[surface_number]
        ...
        if not isinstance(surface.geometry, SegmentedSurface):
            ...
            surface.geometry.cv2 = value
            
    def set_k1(self, value, surface_number):
        """Set the center zone's conic constant of a segmented surface

        by Ziyi Xiong 2025/12
        
        Args:
            value (float): The new value for the k1 coefficient.
            surface_number (int): The index of the surface to modify.
        """
        surface = self.optic.surface_group.surfaces[surface_number]
        ...
        if not isinstance(surface.geometry, SegmentedSurface):
            ...
            surface.geometry.k1 = value
            
    def set_k2(self, value, surface_number):
        """Set the ring zone's conic constant of a segmented surface
        
        by Ziyi Xiong 2025/12
        
        Args:
            value (float): The new value for the k2 coefficient.
            surface_number (int): The index of the surface to modify.
        """
        surface = self.optic.surface_group.surfaces[surface_number]
