...
class GaussianDistribution(BaseDistribution):
    ...
    Generates `num_points` random points drawn from a Gaussian distribution
    with standard deviation `sigma`, truncated to the unit disk.
    ...
    def __init__(self, sigma: float = 0.25, seed=None):
        super().__init__()
        if sigma <= 0:
    ...
    def generate_points(self, num_points: int):
        ...
        while total < num_points:
            batch = max(num_points - total, num_points)
            x = be.random_normal(scale=self.sigma, size=batch, generator=self.rng)
            y = be.random_normal(scale=self.sigma, size=batch, generator=self.rng)
            r2 = x**2 + y**2
            mask = r2 <= 1.0
            if be.any(mask):
                x_valid = x[mask]
                y_valid = y[mask]
                x_chunks.append(x_valid)
                y_chunks.append(y_valid)
                total += int(x_valid.shape[0])

        x_all = be.concatenate(x_chunks)[:num_points]
        y_all = be.concatenate(y_chunks)[:num_points]
        self.x = x_all
        self.y = y_all
