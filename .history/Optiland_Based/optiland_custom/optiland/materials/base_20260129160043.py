...
class BaseMaterial(ABC):
    ...
    def _align_to_wavelength(self, value):
        """
        Aligns the output value device to match the input wavelength device.
        
        Args:
            wavelength: The input wavelength(s), can be a torch tensor.
            value: The output value(s), can be a torch tensor.
        
        Returns:
            The output value(s) aligned to the same device as the input wavelength(s).
        
        by Ziyi Xiong 2026/1 
        """
        if not be.is_torch_tensor(wavelength):
            return value

        import torch

        if be.is_torch_tensor(value) and (
            value.device != wavelength.device or value.dtype != wavelength.dtype
        ):
        ...
        # value is numpy / python scalar / list -> put it onto the same device/dtype
        return torch.as_tensor(value, device=wavelength.device, dtype=wavelength.dtype)
