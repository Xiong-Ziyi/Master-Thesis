...
class BeamShaper:
    ...
    def __init__(
        ...
        if self.backend == "torch":
            ...
            self.R_max = torch.tensor(R_max, dtype=torch.float32, device=self.device)
            self.r_max = torch.tensor(r_max, dtype=torch.float32, device=self.device)
    ...
    @property
    def _B(self) -> float | torch.Tensor:
        """
    ...
    def s_prime(self, r) -> float | torch.Tensor:
        """
    ...
    @property
    def s_prime_zero(self) -> float | torch.Tensor:
        """
    ...
    @property
    def Delta_s_prime(self) -> float | torch.Tensor:
        """
