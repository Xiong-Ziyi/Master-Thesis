...
class BeamShaper:
    ...
    def __init__(
        ...
        # store parameters as backend scalars (torch tensors if torch backend)
        self.omega = self.be.tensor(omega)
        self.R_max = self.be.tensor(R_max)
        self.r_max = self.be.tensor(r_max)
        self.d = self.be.tensor(d)
        self.n = self.be.tensor(n)

    @property
    def _B(self) -> ArrayLike:
    ...
    def s_prime(self, r: ArrayLike) -> ArrayLike:
        r = self.be.tensor(r)
        sqrt = self.be.sqrt(1 - self.be.exp(-2 * r**2 / self.omega**2))
        return self.d / (1 - (self._B / r) * sqrt)

    @property
    def s_prime_zero(self) -> ArrayLike:
        if self.approx:
            rt2 = self.be.sqrt(2.0)  # backend-safe sqrt(2)
            return self.d / (1 - rt2 * self._B / self.omega)
        else:
            eps = self.be.tensor(1e-15)
            return self.s_prime(eps)

    @property
    def Delta_s_prime(self) -> ArrayLike:
        return self.s_prime(self.omega) - self.s_prime_zero

    @property
    def k_1(self) -> ArrayLike:
        coeff = -2 * self.Delta_s_prime * self.s_prime_zero * (self.n - 1) ** 2
        return coeff / (self.omega**2) - self.n**2

    @property
    def r_c1(self) -> ArrayLike:
        r_c1 = -self.s_prime_zero * (self.n - 1)
        if abs(self.be.scalar(r_c1)) < abs(self.be.scalar(self.r_max)):
            print(
                "Warning: The vertex radius of the first surface is smaller than the maximum input radial coordinate."
            )
        return r_c1

    @property
    def k_2(self) -> ArrayLike:
        coeff = self.s_prime_zero * self.k_1 + self.n**2 * self.d
        return coeff / (self.s_prime_zero - self.d)

    @property
    def r_c2(self) -> ArrayLike:
        r_c2 = (self.n - 1) * (self.d - self.s_prime_zero)
        if abs(self.be.scalar(r_c2)) < abs(self.be.scalar(self.R_max)):
            print(
                "Warning: The vertex radius of the second surface is smaller than the maximum output radial coordinate."
            )
        return r_c2

    @property
    def xpan_ratio(self) -> ArrayLike:
        f_1 = self.s_prime_zero
        f_2 = self.d - self.s_prime_zero
        return self.be.abs(f_2 / f_1)

    @property
    def apodization_factor(self) -> ArrayLike:
        return (self.r_max / self.omega) ** 2

    @property
    def result(self) -> dict:
        return {
            "r_c1": self.be.scalar(self.r_c1),
            "r_c2": self.be.scalar(self.r_c2),
            "k_1": self.be.scalar(self.k_1),
            "k_2": self.be.scalar(self.k_2),
        }

class BeamShaperFitter:
    '''
    This class is to fit Galilean or Keplerian beam shapers' two surfaces' sags with even aspheric surface equation, only numpy based.
    ...
    def __init__(self, 
        ...
        self.be = beam_shaper.be
        
        self.omega = self.be.scalar(beam_shaper.omega)
        self.r_max = self.be.scalar(beam_shaper.r_max)
        self.R_max = self.be.scalar(beam_shaper.R_max)
        self.d = self.be.scalar(beam_shaper.d)
        self.n = self.be.scalar(beam_shaper.n)
