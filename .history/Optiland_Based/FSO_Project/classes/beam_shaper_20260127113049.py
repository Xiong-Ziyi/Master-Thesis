...
class BeamShaper:
    ...
    Supported by both numpy and torch.
    ...
    Args:
        ...
        backends (str): either "numpy" or "torch", indicating which backend to use.
        device (str): either "cpu" or "cuda", indicating which device to use for torch backend.
    """
    ...
    def __init__(
        ...
        d: float,
        n: float,
        type: Literal["Keplerian", "Galilean"] = "Keplerian",
        approx: bool = True,
        ...
        if backend == "numpy":
            ...
            self.omega = torch.tensor(omega, device=device, dtype=self.dtype)
            self.R_max = torch.tensor(R_max, device=device, dtype=self.dtype)
            self.r_max = torch.tensor(r_max, device=device, dtype=self.dtype)
            self.d = torch.tensor(d, device=device, dtype=self.dtype)
            self.n = torch.tensor(n, device=device, dtype=self.dtype)
    
    def _scaler(self, x) -> float:
        """
        Convert backend scalar to python float when needed.
        """
    ...
    def _abs_scalar(self, x) -> float:
        """
        abs(x) as a python float, for warning comparison.
        """
        return abs(self._scaler(x))
    
    @property
    def _B(self) -> np.ndarray | torch.Tensor:
        """
        Returns a intermediate coefficient B given by Equation 7.13.
        """
        denom = self.be.sqrt(1 - self.be.exp(-2 * (self.r_max / self.omega) ** 2))
        if self.type == "Keplerian":
            return -self.R_max / denom
        else:
            return self.R_max / denom

    def s_prime(self, r) -> np.ndarray | torch.Tensor:
        """
        Returns the object distance after the first lens at different ray height,
        given by Equation 7.16.
        """
        if self.backend == "torch" and not isinstance(r, torch.Tensor):
            r = torch.tensor(r, device=self.device, dtype=self.dtype)
        
        sqrt = self.be.sqrt(1 - self.be.exp(-2 * r**2 / self.omega**2))
        s_prime = self.d / (1 - (self._B / r) * sqrt)
        return s_prime

    @property
    def s_prime_zero(self) -> np.ndarray | torch.Tensor:
        """
        Returns paraxial object distance, which is also equal to effective focal length f_1,
        given by Equation 7.17 and 7.18.
        """
        if self.approx:
            return self.d / (1 - self.be.sqrt(2) * self._B / self.omega)
        else:
            if self.backend == "torch":
                eps = torch.tensor(1e-15, device=self.device, dtype=self.dtype)
            return self.s_prime(eps)

    @property
    def Delta_s_prime(self) -> np.ndarray | torch.Tensor:
        """
    ...
    @property
    def k_1(self) -> np.ndarray | torch.Tensor:
        """
    ...
    @property
    def r_c1(self) -> np.ndarray | torch.Tensor:
        """
        Returns the vertex radius of the first aspheric surface.
        """
        r_c1 = -self.s_prime_zero * (self.n - 1)
        
        if self._abs_scalar(r_c1) < self._abs_scalar(self.r_max):
            print("Warning: The vertex radius of the first surface is smaller than the maximum input radial coordinate.")
        
        return r_c1

    @property
    def k_2(self) -> np.ndarray | torch.Tensor:
        """
    ...
    @property
    def r_c2(self) -> np.ndarray | torch.Tensor:
        """
        Returns the vertex radius of the second aspheric surface.
        """
        r_c2 = (self.n - 1) * (self.d - self.s_prime_zero)
        
        if self._abs_scalar(r_c2) < self._abs_scalar(self.R_max):
            print("Warning: The vertex radius of the second surface is smaller than the maximum output radial coordinate.")
        
        return r_c2

    @property
    def xpan_ratio(self) -> np.ndarray | torch.Tensor:
        """
        Returns the expansion ratio of the beam shaper.
        """
        f_1 = self.s_prime_zero
        f_2 = self.d - self.s_prime_zero
        return self.be.abs(f_2 / f_1)

    @property
    def apodization_factor(self) -> np.ndarray | torch.Tensor:
        """
    ...
    def result(self) -> dict:
        ...
        """
        return {
            "r_c1": self._scaler(self.r_c1),
            "r_c2": self._scaler(self.r_c2),
            "k_1": self._scaler(self.k_1),
            "k_2": self._scaler(self.k_2),
        }
