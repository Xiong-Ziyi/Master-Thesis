...
class EvenAsphere:
    ...
    def sag(self) -> np.ndarray:
        ...
        '''
        if self.vertex_r == 0:
            raise ValueError("Vertex radius cannot be zero")
        
        sqrt_term = 1 - (1 + self.k) * (self.r / self.vertex_r)**2
        if np.any(sqrt_term < 0):
            raise ValueError(f"Invalid surface definition: sqrt term is negative. Check k={self.k} and vertex_r={self.vertex_r}")
        
        denom = np.sqrt(sqrt_term)
    ...
    def grad_sag(self) -> np.ndarray:
        ...
        sqrt_term = 1 - (1 + self.k) * (self.r / self.vertex_r)**2
        if np.any(sqrt_term <= 0):
            raise ValueError(f"Invalid surface definition: sqrt term is non-positive. Check k={self.k} and vertex_r={self.vertex_r}")
